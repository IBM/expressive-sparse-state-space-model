...
def create_dataset_model_and_train(
    ...
    model, state = create_model(
        model_name,  dataset.data_dim, dataset.label_dim,  classification=classification,  output_step=output_step,  **model_args,  key=modelkey
    )
    filter_spec = jax.tree_util.tree_map(lambda _: True, model)

    dataloaders = dataset.raw_dataloaders

    return train_model(  model_name, dataset_name,  model,  metric,  filter_spec,
        state, dataloaders,  num_steps,  print_steps,  early_stopping_steps,  lr, lr_scheduler, batch_size,  trainkey,  output_parent_dir + "/" + output_dir)